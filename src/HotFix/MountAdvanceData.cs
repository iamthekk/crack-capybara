...
namespace HotFix
{
	public class MountAdvanceData
	{
		...
		public Mount_advanceMount Config { get; private set; }
		...
		public MountAdvanceData(Mount_advanceMount config)
		{
			this.Config = config;
			this.ID = config.id;
			this.IsUnlock = false;
			this.MemberConfig = GameApp.Table.GetManager().GetGameMember_memberModelInstance().GetElementById(config.memberId);
