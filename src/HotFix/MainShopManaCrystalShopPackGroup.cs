...
namespace HotFix
{
	public class MainShopManaCrystalShopPackGroup : MainShopPackGroupBase
	{
		...
		{
			long num;
			this.shopDataModule.GetRefreshCountDownTime(ShopType.ManaCrystal, this.guildGoodsType, out num);
		...
		{
			this.shopDataModule.GetShopConfig(ShopType.ManaCrystal, out this.shopConfig);
			IEnumerable<IntegralShop_goods> enumerable = from x in this.shopDataModule.GetShopItemsConfig(ShopType.ManaCrystal, this.guildGoodsType)
				orderby x.LevelRequirements
				select x;
			this.shopItemConfigList = new List<IntegralShop_goods>();
			foreach (IntegralShop_goods integralShop_goods in enumerable)
			{
		...
		public void UpdateItems()
		{
			...
			int chapterID = GameApp.Data.GetDataModule(DataName.ChapterDataModule).ChapterID;
			for (int k = 0; k < this.shopItemConfigList.Count; k++)
			{
				...
				if (integralShop_goods.LevelRequirements > chapterID)
				{
