...
namespace HotFix
{
	public class HoverEnemyStateBar : BaseHover
	{
		public override HoverType GetHoverType()
		{
			return HoverType.EnemyStateBar;
		}
		...
		protected override void OnInit()
		{
			...
			if (hoverStateBarData != null)
			{
				...
				this.rechargeBar.gameObject.SetActive(hoverStateBarData.isShowRecharge);
			}
			else
			{
			...
			GameApp.Event.RegisterEvent(LocalMessageName.CC_UIGameHover_RefreshShild, new HandlerEvent(this.OnRefreshShild));
		...
		protected override void OnDeInit()
		{
			...
			GameApp.Event.UnRegisterEvent(LocalMessageName.CC_UIGameHover_RefreshShild, new HandlerEvent(this.OnRefreshShild));
		...
		private void OnRefreshShild(object sender, int type, BaseEventArgs args)
		{
