...
namespace HotFix
{
	public class UIRelicSpecialNode : CustomBehaviour
	{
		...
		protected override void OnInit()
		{
			...
			for (int i = 0; i < this.m_datas.Count; i++)
			{
				...
				if (relicData != null)
				{
					UIRelicSpecialChildNode uirelicSpecialChildNode = Object.Instantiate<UIRelicSpecialChildNode>(this.m_nodePrefab);
					uirelicSpecialChildNode.transform.SetParentNormal(this.m_content, false);
					uirelicSpecialChildNode.SetActive(true);
					uirelicSpecialChildNode.SetData(relicData);
					uirelicSpecialChildNode.Init();
					this.m_dic[uirelicSpecialChildNode.GetObjectInstanceID()] = uirelicSpecialChildNode;
				}
			}
		...
		protected override void OnDeInit()
		{
			...
			foreach (KeyValuePair<int, UIRelicSpecialChildNode> keyValuePair in this.m_dic)
			{
		...
		[SerializeField]
		private UIRelicSpecialChildNode m_nodePrefab;

		public Dictionary<int, UIRelicSpecialChildNode> m_dic = new Dictionary<int, UIRelicSpecialChildNode>();
	}
}
