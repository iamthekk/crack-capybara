...
namespace HotFix
{
	public class UIMainButton_Activity : BaseUIMainButton
	{
		...
		protected override void OnInit()
		{
			...
			RedPointController.Instance.RegRecordChange("Main.Activity_Week", new Action<RedNodeListenData>(this.OnRedPointChange));
		}

		protected override void OnDeInit()
		{
			RedPointController.Instance.UnRegRecordChange("Main.Activity_Week", new Action<RedNodeListenData>(this.OnRedPointChange));
		...
		{
			GameApp.View.OpenView(ViewName.ActivityWeekEntryModule, null, 1, null, null);
		}
		...
		private void OnRefreshNameText()
		{
			...
			{
				this.m_nameTxt.text = Singleton<LanguageManager>.Instance.GetInfoByID(GameApp.Table.GetManager().GetFunction_Function(102).nameID);
			}
		}
		...
		public const string REDPOINT_NAME = "Main.Activity_Week";
	}
}
