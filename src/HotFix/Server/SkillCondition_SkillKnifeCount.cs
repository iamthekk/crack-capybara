...
namespace Server
{
	public class SkillCondition_SkillKnifeCount : ConditionBase
	{
		...
		{
			this.m_data = ((!string.IsNullOrEmpty(parameter)) ? JsonManager.ToObject<SkillCondition_SkillKnifeCount.Data>(parameter) : new SkillCondition_SkillKnifeCount.Data());
		}
		...
		{
			if (this.m_data.KnifeTypes == null)
			{
				HLog.LogError(string.Format("技能触发条件错误. SkillID = {0}", skill.skillData.m_id));
				return;
			}
			if (!(skill is SSkill_Knife))
			{
				return;
			}
			SSkill_Knife sskill_Knife = skill as SSkill_Knife;
			for (int i = 0; i < this.m_data.KnifeTypes.Length; i++)
			{
				if (sskill_Knife.knifeType == this.m_data.KnifeTypes[i])
				{
		...
		public SkillCondition_SkillKnifeCount.Data m_data;
		...
		public class Data
		{
			public SSkill_Knife.KnifeType[] KnifeTypes;
