...
{
	public sealed class RGuildBossCombatResp : IMessage
	{
		[DebuggerNonUserCode]
		public static MessageParser<RGuildBossCombatResp> Parser
		{
			get
			{
				return RGuildBossCombatResp._parser;
			}
		}
		...
		public void WriteTo(CodedOutputStream output)
		{
			...
			this.startUnits_.WriteTo(output, RGuildBossCombatResp._repeated_startUnits_codec);
			this.endUnits_.WriteTo(output, RGuildBossCombatResp._repeated_endUnits_codec);
			if (this.Power != 0L)
			{
		...
		public int CalculateSize()
		{
			...
			num += this.startUnits_.CalculateSize(RGuildBossCombatResp._repeated_startUnits_codec);
			num += this.endUnits_.CalculateSize(RGuildBossCombatResp._repeated_endUnits_codec);
			if (this.Power != 0L)
			{
			...
			while ((num = input.ReadTag()) != 0U)
			{
				if (num <= 42U)
				{
					...
					else
					{
						...
						if (num == 42U)
						{
							this.startUnits_.AddEntriesFrom(input, RGuildBossCombatResp._repeated_startUnits_codec);
							continue;
						}
					}
				}
				else if (num <= 64U)
				{
					if (num == 50U)
					{
						this.endUnits_.AddEntriesFrom(input, RGuildBossCombatResp._repeated_endUnits_codec);
						continue;
					}
					if (num == 56U)
					{
		...
		private static readonly MessageParser<RGuildBossCombatResp> _parser = new MessageParser<RGuildBossCombatResp>(() => new RGuildBossCombatResp());
		...
		public const int BossHpFieldNumber = 11;
