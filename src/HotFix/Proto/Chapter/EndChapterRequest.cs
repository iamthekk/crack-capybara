...
{
	public sealed class EndChapterRequest : IMessage
	{
		[DebuggerNonUserCode]
		public static MessageParser<EndChapterRequest> Parser
		{
			get
			{
				return EndChapterRequest._parser;
			}
		}
		...
		public void WriteTo(CodedOutputStream output)
		{
			...
			this.serverRewards_.WriteTo(output, EndChapterRequest._repeated_serverRewards_codec);
			this.monsterRewards_.WriteTo(output, EndChapterRequest._repeated_monsterRewards_codec);
			this.skillIds_.WriteTo(output, EndChapterRequest._repeated_skillIds_codec);
		}
		...
		public int CalculateSize()
		{
			...
			num += this.serverRewards_.CalculateSize(EndChapterRequest._repeated_serverRewards_codec);
			num += this.monsterRewards_.CalculateSize(EndChapterRequest._repeated_monsterRewards_codec);
			return num + this.skillIds_.CalculateSize(EndChapterRequest._repeated_skillIds_codec);
		}
		...
		public void MergeFrom(CodedInputStream input)
		{
			...
			while ((num = input.ReadTag()) != 0U)
			{
				...
				else if (num <= 58U)
				{
					...
					if (num == 58U)
					{
						this.serverRewards_.AddEntriesFrom(input, EndChapterRequest._repeated_serverRewards_codec);
						continue;
					}
				}
				else
				{
					if (num == 66U)
					{
						this.monsterRewards_.AddEntriesFrom(input, EndChapterRequest._repeated_monsterRewards_codec);
						continue;
					}
					if (num == 72U || num == 74U)
					{
						this.skillIds_.AddEntriesFrom(input, EndChapterRequest._repeated_skillIds_codec);
						continue;
					}
				}
				input.SkipLastField();
		...
		private static readonly MessageParser<EndChapterRequest> _parser = new MessageParser<EndChapterRequest>(() => new EndChapterRequest());
		...
		private static readonly FieldCodec<int> _repeated_skillIds_codec = FieldCodec.ForInt32(74U);
