...
{
	public sealed class StartChapterResponse : IMessage
	{
		[DebuggerNonUserCode]
		public static MessageParser<StartChapterResponse> Parser
		{
			get
			{
				return StartChapterResponse._parser;
			}
		}
		...
		public void WriteTo(CodedOutputStream output)
		{
			...
			this.eventMap_.WriteTo(output, StartChapterResponse._map_eventMap_codec);
			this.activeMap_.WriteTo(output, StartChapterResponse._map_activeMap_codec);
			if (this.BattleKey.Length != 0)
			{
				output.WriteRawTag(66);
				output.WriteString(this.BattleKey);
			}
			if (this.BattleTimes != 0U)
			{
				output.WriteRawTag(72);
				output.WriteUInt32(this.BattleTimes);
			}
		}
		...
		public int CalculateSize()
		{
			...
			num += this.eventMap_.CalculateSize(StartChapterResponse._map_eventMap_codec);
			num += this.activeMap_.CalculateSize(StartChapterResponse._map_activeMap_codec);
			if (this.BattleKey.Length != 0)
			{
			...
			if (this.BattleTimes != 0U)
			{
				num += 1 + CodedOutputStream.ComputeUInt32Size(this.BattleTimes);
			}
			return num;
		}
		...
		public void MergeFrom(CodedInputStream input)
		{
			...
			while ((num = input.ReadTag()) != 0U)
			{
				...
				else if (num <= 50U)
				{
					...
					if (num == 50U)
					{
						this.eventMap_.AddEntriesFrom(input, StartChapterResponse._map_eventMap_codec);
						continue;
					}
				}
				else
				{
					if (num == 58U)
					{
						this.activeMap_.AddEntriesFrom(input, StartChapterResponse._map_activeMap_codec);
						continue;
					}
					if (num == 66U)
					{
					...
					if (num == 72U)
					{
						this.BattleTimes = input.ReadUInt32();
						continue;
					}
				}
				input.SkipLastField();
		...
		private static readonly MessageParser<StartChapterResponse> _parser = new MessageParser<StartChapterResponse>(() => new StartChapterResponse());
		...
		public const int BattleKeyFieldNumber = 8;

		private string battleKey_ = "";

		public const int BattleTimesFieldNumber = 9;

		private uint battleTimes_;
	}
}
