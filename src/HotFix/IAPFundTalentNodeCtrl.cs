...
namespace HotFix
{
	public class IAPFundTalentNodeCtrl : IAPFundCtrlBase
	{
		...
		public override void OnShow()
		{
			...
			this.GroupData = this.mDataModule.LevelFund.GetCurrentFundGroup(IAPLevelFundGroupKind.TalentLevel);
			this.RefreshUI();
		...
		public void RefreshUI()
		{
			...
			this.Text_TotalDiamonds.text = this.GroupData.GetTotalDiamondsCount().ToString();
		...
		{
			GameApp.View.CloseView(ViewName.IAPFundTalentViewModule, null);
		}
		...
		private void OnCollectAll()
		{
			...
			NetworkUtils.Purchase.LevelFundRewardRequest(canCollectList, delegate(bool result, LevelFundRewardResponse resp)
			{
				if (result)
				{
					...
					GameApp.SDK.Analyze.Track_InvestGet("天赋基金", this.GroupData.GroupID, this.GroupData.Level, resp.CommonData.Reward, this.GroupData.PurchaseID, freeCollectedRewardedList, payCollectedRewardedList);
				}
			});
		}
		...
		private void PlayScale()
		{
			...
			if (!this.playui)
			{
				for (int i = 0; i < this.LoopList.ShownItemCount; i++)
				{
