...
namespace HotFix
{
	public class IAPFundTowerNodeCtrl : IAPFundCtrlBase
	{
		...
		public override void OnShow()
		{
			...
			this.GroupData = this.mDataModule.LevelFund.GetCurrentFundGroup(IAPLevelFundGroupKind.TowerLevel);
			this.RefreshUI();
		...
		public void RefreshUI()
		{
			...
			this.Text_TotalDiamonds.text = this.GroupData.GetTotalKeyCount(62).ToString();
		...
		{
			GameApp.View.CloseView(ViewName.IAPFundTowerViewModule, null);
		}
		...
		private void OnCollectAll()
		{
			...
			NetworkUtils.Purchase.LevelFundRewardRequest(canCollectList, delegate(bool result, LevelFundRewardResponse resp)
			{
				if (result)
				{
					...
					GameApp.SDK.Analyze.Track_InvestGet("挑战之塔基金", this.GroupData.GroupID, this.GroupData.Level, resp.CommonData.Reward, this.GroupData.PurchaseID, freeCollectedRewardedList, payCollectedRewardedList);
				}
			});
		}
