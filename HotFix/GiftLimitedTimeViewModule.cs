...
namespace HotFix
{
	public class GiftLimitedTimeViewModule : BaseViewModule
	{
		...
		public override void OnOpen(object data)
		{
			...
			PurchaseCommonData.PurchaseData purchaseData = this.m_iapDataModule.LimitedTimeGift.GetPurchaseData();
			if (purchaseData == null)
			{
				HLog.LogError("GiftLimitedTimeViewModule GetDataID is null");
				return;
			}
			IAP_PushPacks elementById = GameApp.Table.GetManager().GetIAP_PushPacksModelInstance().GetElementById(purchaseData.m_id);
			if (elementById == null)
			{
				HLog.LogError(string.Format("GiftLimitedTimeViewModule pushTable is null,id ={0}", purchaseData.m_id));
				return;
			}
		...
		{
			if (this.m_iapDataModule.LimitedTimeGift.GetLastTime() <= 0L)
			{
		...
		{
			string lastTimeString = this.m_iapDataModule.LimitedTimeGift.GetLastTimeString();
		...
		{
			if (this.m_iapDataModule.LimitedTimeGift.IsEnable())
			{
				return;
			}
			this.OnClickCloseBt();
		...
		{
			GameApp.View.CloseView(ViewName.GiftLimitedTimeViewModule, null);
		}
