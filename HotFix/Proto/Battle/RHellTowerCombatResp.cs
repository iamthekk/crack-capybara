...
{
	public sealed class RHellTowerCombatResp : IMessage
	{
		[DebuggerNonUserCode]
		public static MessageParser<RHellTowerCombatResp> Parser
		{
			get
			{
				return RHellTowerCombatResp._parser;
			}
		}
		...
		public void WriteTo(CodedOutputStream output)
		{
			...
			this.startUnits_.WriteTo(output, RHellTowerCombatResp._repeated_startUnits_codec);
			this.endUnits_.WriteTo(output, RHellTowerCombatResp._repeated_endUnits_codec);
			if (this.Power != 0L)
			{
		...
		public int CalculateSize()
		{
			...
			num += this.startUnits_.CalculateSize(RHellTowerCombatResp._repeated_startUnits_codec);
			num += this.endUnits_.CalculateSize(RHellTowerCombatResp._repeated_endUnits_codec);
			if (this.Power != 0L)
			{
			...
			while ((num = input.ReadTag()) != 0U)
			{
				if (num <= 42U)
				{
					...
					else
					{
						...
						if (num == 34U)
						{
							this.startUnits_.AddEntriesFrom(input, RHellTowerCombatResp._repeated_startUnits_codec);
							continue;
						}
						if (num == 42U)
						{
							this.endUnits_.AddEntriesFrom(input, RHellTowerCombatResp._repeated_endUnits_codec);
							continue;
						}
					}
				}
		...
		private static readonly MessageParser<RHellTowerCombatResp> _parser = new MessageParser<RHellTowerCombatResp>(() => new RHellTowerCombatResp());
