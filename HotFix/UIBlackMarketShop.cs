...
namespace HotFix
{
	public class UIBlackMarketShop : UIBaseMainPageNode
	{
		...
		private void CommonShow()
		{
			...
			RedPointController.Instance.RegRecordChange("Main.BlackMarket", new Action<RedNodeListenData>(this.OnRedPointChange_BlackMarket));
			this.tabList[2].gameObject.SetActive(Singleton<GameFunctionController>.Instance.IsFunctionOpened(FunctionID.ManaCrystalShop, false));
			if (!this.isGuildShop)
			{
		...
		protected override void OnHide()
		{
			...
			RedPointController.Instance.UnRegRecordChange("Main.BlackMarket", new Action<RedNodeListenData>(this.OnRedPointChange_BlackMarket));
		...
		private void OnRedPointChange_BlackMarket(RedNodeListenData redData)
		{
			RedNodeOneCtrl redNodeOneCtrl;
			if (this.redNodeDic.TryGetValue(MainShopType.BlackMarket, out redNodeOneCtrl))
			{
