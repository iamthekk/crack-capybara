...
namespace HotFix
{
	public class ChatServerSubPanel : ChatHomeSubPanel
	{
		public override ChatHomeSubPanelType PanelType
		{
			get
			{
				return ChatHomeSubPanelType.Server;
			}
		}
		...
		{
			this.chatPanel.Init();
			this.chatPanel.OutputUI.HistoryRequire = new ChatServerSubPanel.SceneHistoryRequire();
			this.chatPanel.InputUI.SendInterval = (float)Singleton<GameConfig>.Instance.ServerChatInterval;
		...
		protected override void OnSelect()
		{
			...
			this.chatPanel.SetChatGroup(SocketGroupType.Server, ChatProxy.ServerChat.GetGroupID());
		}
		...
		private class SceneHistoryRequire : ChatHistoryMessageRequire
		{
			public override void RequireHistory(ChatData chat, Action<bool> callback)
			{
				...
				socketNetProxy.GetMessageHistory(num, SocketGroupType.Server, ChatProxy.ServerChat.GetGroupID(), delegate(bool result, ChatGetMessageRecordsResponse resp)
				{
